...
import com.llmintentions.device.databinding.SheetExecuteBinding
...
class ToolExecuteSheet : BottomSheetDialogFragment() {
    ...
    private fun execute() {
            ...
            for (param in toolDef.params) {
                ...
                when (input) {
                    ...
                    is TextInputEditText -> {
                        ...
                        when (param.type) {
                            ParamType.INTEGER -> text.toLongOrNull()?.let { put(param.name, it) }
        ...
        scope.launch {
            var isError = false
            val result = try {
                withContext(Dispatchers.IO) { handler(args) }
            } catch (e: Exception) {
                isError = true
                "Error: ${e.message}"
            }

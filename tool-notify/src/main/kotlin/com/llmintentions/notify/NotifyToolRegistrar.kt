    ...
    fun register(registry: ToolRegistry, ctx: Context) {
        ...
        registry.textTool("notifications_list", "List all currently active notifications on the device",
            ...
            val list = notifications.take(limit).map { sbn ->
                buildJsonObject {
                    ...
                    put("has_reply_action", sbn.notification.actions?.any { it.remoteInputs?.isNotEmpty() == true } == true)
                }
            }
            buildJsonObject {
                put("count", list.size)
        ...
        registry.textTool("notification_filter", "Search active notifications by app or text",
            ...
            val list = notifications.mapNotNull { sbn ->
                ...
                if (textFilter != null && !title.lowercase().contains(textFilter) && !text.lowercase().contains(textFilter)) return@mapNotNull null
                buildJsonObject {
                    put("key", sbn.key)
                    put("package", sbn.packageName)
                    put("time", df.format(Date(sbn.postTime)))
